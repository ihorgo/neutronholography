...
//  h.hFCCc=Four2DBgHolo(h.hFCCc,5,1);
  ...
  save('c:\mainsci\macros\psi08\calcpsibi.dat',h);
  mclose('all');
...
//  load('c:\mainsci\macros\psi08\psipartsmod.dat');
  ...
  save('c:\mainsci\macros\psi08\recpsiphcalc.dat',H1);
  clear H1;
  mclose('all');

  load('c:\mainsci\macros\psi08\recpsiphcalc.dat');
//  H1s=SymmRecHolo(H1,24);
//  save('c:\mainsci\macros\psi08\recpsiph1.dat',H1,H1s);
  H1s=HoloCubeSymm(H1,1);
  save('c:\mainsci\macros\psi08\recpsiphcalc.dat',H1,H1s);
  HH.H=H1;
  HH.Hs=H1s;
  clear H1;
  clear H1s;
